...
const CATEGORY_ICONS: Record<string, any> = {
  "date-night": Coffee, groceries: ShoppingCart, cleaning: Tv, travel: Cake, family: Cake, bills: Cake,
};
const CATEGORY_ICON_BG: Record<string, string> = {
  "date-night": "bg-primary/15", groceries: "bg-success/15", cleaning: "bg-accent/15",
  travel: "bg-warning/15", family: "bg-secondary", bills: "bg-muted",
};
const CATEGORY_LABEL: Record<string, string> = {
  "date-night": "Romance", groceries: "Household", cleaning: "Cleaning", bills: "Bills", travel: "Travel", family: "Family",
};
...
export default function CalendarPage() {
  ...
  const addEvent = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const newEvent = {
      ...
      event_time: (fd.get("time") as string) || null,
      assigned_to: fd.get("assignedTo") as string,
      ...
      <div className="px-5 pt-10 pb-6">
        ...
        <div className="flex items-center justify-between mb-1">
          <div>
            ...
            <p className="text-xs text-muted-foreground flex items-center gap-1"><span>♥</span> Our Love Timeline</p>
          </div>
        ...
        <div className="flex gap-2 mt-4 mb-5">
          {(["day", "week", "month"] as ViewMode[]).map(mode => (
            <button key={mode} onClick={() => setViewMode(mode)}
              className={`px-4 py-1.5 rounded-full text-xs font-medium transition-colors flex items-center gap-1.5 ${viewMode === mode ? "bg-[hsl(100,20%,72%)] text-foreground" : "bg-card shadow-card text-muted-foreground"}`}>
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto pb-1">
          {days.map(day => {
            ...
            return (
              <button key={day.toISOString()} onClick={() => { setSelectedDate(day); setCurrentDate(day); }}
                className={`flex flex-col items-center min-w-[52px] py-2.5 px-2 rounded-2xl transition-all ${isSelected ? "bg-[hsl(100,20%,72%)] shadow-soft" : isToday(day) ? "bg-card shadow-card" : ""}`}>
                <span className={`text-[10px] font-medium mb-1 ${isSelected ? "text-foreground" : "text-muted-foreground"}`}>{format(day, "EEE")}</span>
                <span className="text-base font-bold text-foreground">{format(day, "d")}</span>
        ...
        {dayEvents.length === 0 ? (
          ...
          <div className="space-y-3">
            {dayEvents.map(event => {
              ...
              return <EventCard key={event.id} icon={IconComp} iconBg={iconBg} title={event.title} time={event.event_time || "All day"} badge={CATEGORY_LABEL[event.category] || event.category} onDelete={() => deleteEvent(event.id)} />;
            })}
          </div>
        )}
        ...
        <div className="bg-card rounded-2xl p-6 shadow-card text-center mt-5 border border-border">
          ...
          <button onClick={() => setShowAdd(true)} className="px-5 py-2 rounded-full bg-foreground text-background text-xs font-semibold inline-flex items-center gap-1.5">
        ...
        <div className="bg-[hsl(38,40%,88%)] rounded-2xl px-4 py-3.5 mt-4 flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-[hsl(38,40%,78%)] flex items-center justify-center shrink-0"><Cake size={18} className="text-foreground" /></div>
        ...
        <div className="flex justify-end mt-5">
          <button onClick={() => setShowAdd(true)} className="bg-foreground text-background px-5 py-3 rounded-full flex items-center gap-2 shadow-elevated text-sm font-semibold">
        ...
        <AnimatePresence>
          {showAdd && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-foreground/30 z-50 flex items-end justify-center" onClick={() => setShowAdd(false)}>
              <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 28, stiffness: 300 }} onClick={e => e.stopPropagation()} className="bg-card w-full max-w-lg rounded-t-2xl p-5 shadow-elevated">
                ...
                <form onSubmit={addEvent} className="space-y-3">
                  ...
                  <button type="submit" className="w-full h-12 rounded-xl love-gradient text-primary-foreground font-semibold text-sm shadow-soft">Add Event</button>
                </form>
              </motion.div>
            </motion.div>
          )}
...
function EventCard({ icon: Icon, iconBg, title, time, badge, onDelete }: {
  ...
  return (
    <div className="bg-card rounded-2xl px-4 py-3.5 shadow-card flex items-center gap-3 border border-border">
      <div className={`w-10 h-10 rounded-xl ${iconBg} flex items-center justify-center shrink-0`}><Icon size={18} className="text-foreground" /></div>
